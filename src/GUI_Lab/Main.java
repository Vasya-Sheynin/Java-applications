...
public class Main {
    ...
    static String filename    = "ConversData.dat";
    static String filenameBak = "ConversData.~dat";
    static String idxname     = "ConversData.idx";
    static String idxnameBak  = "ConversData.~idx";
    ...
    static void appendFile(Object[] vals, Boolean zipped)
            throws FileNotFoundException, IOException, ClassNotFoundException {
        try (Index idx = Index.load(idxname);
             RandomAccessFile raf = new RandomAccessFile(filename, "rw")) {

            ConversData conversData = new ConversData();
            conversData.personFullName = (String) vals[0];
            conversData.telNumber = (String) vals[1];
            conversData.date = (String) vals[2];
            conversData.tariff = (String) vals[3];
            conversData.sale = (String) vals[4];
            conversData.beginTime = (String) vals[5];
            conversData.endTime = (String) vals[6];
    ...
    private static ArrayList<String> printRecord(RandomAccessFile raf, String key,
                                     IndexBase pidx, ArrayList<String> data) throws ClassNotFoundException, IOException {
        Long[] poss = pidx.get(key);
        for (long pos : poss) {
            System.out.print("*** Key: " +  key + " points to");
            data.add(printRecord(raf, pos));
        }

        return data;
    }

    static ArrayList<String> printFile()
            throws FileNotFoundException, IOException, ClassNotFoundException {
        long pos;
        int rec = 0;
    ...
    static boolean printFile(String[] args, boolean reverse)
        ...
        try (Index idx = Index.load(idxname);
            ...
            for (String key : keys) {
                printRecord(raf, key, pidx, new ArrayList<>());
            }
        }
        return true;
    }

    static ArrayList<String> printFileData(String[] args, boolean reverse)
            throws ClassNotFoundException, IOException {
        ...
        try (Index idx = Index.load(idxname);
            ...
            for (String key : keys) {
                ...
                data.addAll(printRecord(raf, key, pidx, tmp));
            }
        }
        return data;
    }
    ...
    static boolean findByKey(String[] args)
        ...
        try (Index idx = Index.load(idxname);
            ...
            printRecord(raf, args[2], pidx, new ArrayList<>());
        }
        return true;
    }
    ...
    static ArrayList<String> findByKeyData(String[] args)
            throws ClassNotFoundException, IOException {
        ArrayList<String> data = new ArrayList<>();
        if (args.length != 3) {
            System.err.println("Invalid number of arguments");
        ...
        try (Index idx = Index.load(idxname);
            ...
            printRecord(raf, args[2], pidx, data);
        }
        return data;
    }

    static ArrayList<String> findByKeyData(String[] args, Comparator<String> comp)
            throws ClassNotFoundException, IOException {
        ArrayList<String> data = new ArrayList<>();
        if (args.length != 3) {
            System.err.println("Invalid number of arguments");
